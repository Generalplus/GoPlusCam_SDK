...
#define QUEUE_WAIT_TIMEOUT_SEC     1
#define QUEUE_WAIT_TIMEOUT_USEC    250000
...
public:
    ...
    CType* PopObject(bool bWait = false)
    {
        ...
        if(bWait && m_Queue.size()==0)
        {
            ...
            timeToWait.tv_nsec = (now.tv_usec + QUEUE_WAIT_TIMEOUT_USEC) * 1000;
            
            timeToWait.tv_sec += timeToWait.tv_nsec / NSEC_PER_SEC;
            timeToWait.tv_nsec = timeToWait.tv_nsec % NSEC_PER_SEC;
    ...
    void RemoveAt(int i)
    {
        ...
        Queue_iterator iterator = m_Queue.begin();
