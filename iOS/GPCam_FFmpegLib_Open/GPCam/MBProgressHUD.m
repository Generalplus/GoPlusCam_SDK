...
- (void)setupLabels {
    ...
    [abortButton setTitle: NSLocalizedString(@"Abort", @"") forState: UIControlStateNormal];
    [abortButton addTarget:self action:@selector(PressAbort:) forControlEvents:UIControlEventTouchDown];
    abortButton.titleLabel.font = [UIFont systemFontOfSize:18];
    abortButton.contentHorizontalAlignment = UIControlContentHorizontalAlignmentLeft;
    [abortButton setTitleColor:[UIColor redColor] forState:UIControlStateNormal];
...
- (void)layoutSubviews {
    ...
    CGSize ButtonlabelSize = MB_TEXTSIZE(abortButton.titleLabel.text, abortButton.titleLabel.font);
    if(abortBlock)
    {
        totalSize.width = MAX(totalSize.width, ButtonlabelSize.width);
        totalSize.height += ButtonlabelSize.height;
    }
    ...
    {
        CGRect abortButtonF;
        abortButtonF.origin.y = yPos;
        abortButtonF.origin.x = round((bounds.size.width - ButtonlabelSize.width) / 2) + xPos ;
        abortButtonF.size = ButtonlabelSize;
        abortButton.frame  = abortButtonF;
    }
    else
    {
