...
bool C_VideoEncoder::CreateEncodeRawStream(char *ptszPath,float fFPS,bool bIsRTSP,bool bUsingLocalTime)
{
    ...
    if(m_EncodeInfo.Container == E_EncodeContainer_AVI)
        ...
        m_outStrm->r_frame_rate = pVideoStream->r_frame_rate;
    ...
    if(m_pAudioCodecCtx)
    {
        ...
        m_outAudioStrm->time_base = pAudioStream->time_base;
    }
...
bool C_VideoEncoder::writeRawStream()
{
    ...
    do
    {
        if(pPacket->stream_index == m_pContextManager->GetVideoStreamID())
        {
        ...
        else if(pPacket->stream_index == m_pContextManager->GetAudioStreamID())
        {
...
void C_VideoEncoder::writeRawVideoPacket(AVPacket *pPacket)
{
    ...
    if(m_EncodeInfo.Container == E_EncodeContainer_AVI)
    {
        ...
        if(m_bIsRTSP)
        {
            // Remove header which appended by ffmpeg and keep JFIF
            /*av_new_packet(tempkt,pPacket->size - COPY_SIZE);//Do not using av_copy_packet(),av_copy_packet() will share same buffer address not alloc new one.
            memcpy(&tempkt->data[0x00],&pPacket->data[0x00], JFIF_TAG_SIZE); //Must keep JFIF for avi file
            memcpy(&tempkt->data[JFIF_TAG_SIZE],&pPacket->data[SKIP_SIZE], (pPacket->size - COPY_SIZE));
            */
        ...
        {
            //Append JFIF
            /*const uint8_t JFIF[JFIF_TAG_SIZE] = { 0xFF ,0xD8 ,0xFF ,0xE0 ,0x00 ,0x10 ,0x4A ,0x46 ,0x49 ,0x46
                                                 ,0x00 ,0x02 ,0x01 ,0x00 ,0x00 ,0x01 ,0x00 ,0x01 ,0x00 ,0x00};
            
            av_new_packet(tempkt,pPacket->size + JFIF_TAG_SIZE);
            memcpy(&tempkt->data[0x00],JFIF, JFIF_TAG_SIZE);
            memcpy(&tempkt->data[JFIF_TAG_SIZE],&pPacket->data[0], pPacket->size);*/
